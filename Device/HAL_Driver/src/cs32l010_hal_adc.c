...
HAL_StatusTypeDef HAL_ADC_PollForConversion(ADC_HandleTypeDef* hadc, uint32_t Timeout)
{
  ...
  /*    can be set.                                                           */
	if(HAL_IS_BIT_CLR(hadc->Instance->CR2, ADC_CR2_CIRCLE_MODE))  // ��ADCѭ��ת��ģʽ
	{
		if (HAL_IS_BIT_CLR(hadc->Instance->CR1, ADC_CR1_CT)) // ������ת��ģʽ 
		{
			/* Wait until End of Single Conversion START flag is cleared */
			while(HAL_IS_BIT_SET(hadc->Instance->CR0, ADC_CR0_START)) // ADCת���Ƿ����
			{
...
//				if(Timeout != HAL_MAX_DELAY)
		...
		else
		{
			...
			while(__HAL_ADC_GET_RAWFLAG(hadc, ADC_RAWINTFLAG_CONTINUE) == RESET) // ��ȡADC����ת����ɱ�־  
			{
				/* Check if timeout is disabled (set to infinite wait) */
//				if(Timeout != HAL_MAX_DELAY)
				if(Timeout == HAL_MAX_DELAY)
...
uint32_t HAL_ADC_GetAccValue(ADC_HandleTypeDef* hadc)
{
  ...
  assert_param(IS_ADC_ALL_INSTANCE(hadc->Instance));
	...
	SET_BIT(hadc->Instance->CR1, ADC_CR1_RACC_CLR); // AccҪ��0
